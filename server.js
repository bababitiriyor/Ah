...
const MOB_TYPES = [
  { shape: 'wolf', color: '#6b4932', outline: '#28170d', eyes: '#ffcc66', typeName: '🐺 Kurt', radius: 46, hp: 300, dmg: 28, speed: 18, wanderSpeed: 10, xpReward: 80, goldReward: 22 },
  { shape: 'scorpion', color: '#4a2818', outline: '#1a0d06', eyes: '#ff4400', typeName: '🦂 Akrep', radius: 52, hp: 520, dmg: 46, speed: 15, wanderSpeed: 9, xpReward: 150, goldReward: 38 },
  { shape: 'bear', color: '#4a2f1b', outline: '#1a1008', eyes: '#ffaa00', typeName: '🐻 Ayı', radius: 65, hp: 950, dmg: 72, speed: 14, wanderSpeed: 8, xpReward: 320, goldReward: 82 },
  { shape: 'spider', color: '#2a1a38', outline: '#0f0814', eyes: '#ff1100', typeName: '🕷️ Örümcek', radius: 48, hp: 380, dmg: 34, speed: 17, wanderSpeed: 9, xpReward: 100, goldReward: 28 },
];
...
const CHEST_CONFIG = {
  ...
  gold_chest: { cost: 1200, rewards: ['dragon', 'phoenix', 'skin_aurora', 'skin_storm', 'skin_sapphire', 'skin_ruby', 'skin_frostwolf', 'skin_cyber', 'yeti', 'robot', 'ninja', 'kiz_orman'] },
  rare_chest: { cost: 2400, rewards: ['dragon', 'phoenix', 'kiz_ates', 'kiz_buz', 'skin_void', 'skin_moon', 'skin_amethyst', 'skin_icefire', 'skin_reef', 'skin_shadow', 'skin_thunder', 'yeti', 'robot', 'shark'] },
  ame_chest: { cost: 4200, rewards: ['kiz_ates', 'kiz_buz', 'kiz_samurai', 'skin_magma', 'skin_solar', 'skin_icefire', 'skin_thunder', 'skin_chroma', 'skin_void', 'kiz_karanlik', 'phoenix', 'dragon'] }
};
...
async function handleApi(request, response, requestPath) {
  ...
  if (requestPath === '/api/profile/xp' && request.method === 'POST') {
    if (!user) {
      ...
      sendJson(response, 200, {
        ok: true, isGuest: true, gainedXp, coinsEarned,
  ...
  if (requestPath === '/api/daily-reward/claim' && request.method === 'POST') {
    ...
    user.gold = user.coins;
    user.dailyReward = {
      day: state.day >= DAILY_REWARDS.length ? 1 : state.day + 1,
      claimedDate: today
    };
    saveAccountData(true);
    sendJson(response, 200, {
...
function capturePlayerInTrap(target) {
  ...
  for (const building of buildings.values()) {
    ...
    if (owner && ((owner.clanId && owner.clanId === target.clanId) || (owner.team && target.team && owner.team === target.team))) continue;
...
function pushTrappedVictim(owner, target, dx, dy, requestedStep = 1) {
  ...
  const now = Date.now();
  if (now - (trapPushCooldowns.get(cooldownKey) || 0) < 50) return false;
  trapPushCooldowns.set(cooldownKey, now);
  const step = Math.min(1.5, Math.max(0.25, Number(requestedStep) || 1));
  const pushX = (dx / length) * step;
...
function resolveTrapOwnerCollisions(owner) {
  ...
  for (const target of players.values()) {
    ...
    const minDistance = (Number(owner.radius) || 34) + (Number(target.radius) || 34);
    if (distance < minDistance) pushTrappedVictim(owner, target, dx, dy, (minDistance - distance) * 0.35);
  }
}
...
setInterval(() => {
  ...
  for (const mob of mobs.values()) {
    ...
    if (target && !target.visibleMobIds?.has(mob.id)) target = null;
    ...
    if (!target) {
      ...
      for (const candidate of players.values()) {
        ...
        if (!candidate.visibleMobIds?.has(mob.id)) continue;
    ...
    for (const b of nearbyBuildings(mob.x, mob.y, mob.radius + 120)) {
      ...
      if (bdist2 < minBdist * minBdist && bdist2 > 0) {
        ...
        if (b.type === 3 && (b.hp ?? 100) > 0) { // Spike damage
          ...
          mob.stateSeq = (mob.stateSeq || 0) + 1;
          io.emit('mob_update', { id: mob.id, seq: mob.stateSeq, ts: mob.stateAt, hp: mob.hp, maxHp: mob.maxHp, hitFlash: 8 });
          if (mob.hp <= 0) {
            mobs.delete(mob.id);
            ...
            if (owner) {
              const trapGold = Math.max(1, Math.floor((mob.goldReward || 10) * 0.65));
              owner.gold = (owner.gold || 0) + trapGold;
              owner.score = (owner.score || 0) + trapGold * 3;
...
io.on('connection', (socket) => {
  ...
  socket.on('state', (data = {}) => {
    ...
    for (const key of ['kills', 'gold']) {
      if (Number.isFinite(Number(data[key]))) player[key] = Math.max(0, Number(data[key]));
    }
    if (Number.isFinite(Number(data.xp))) {
      player.xp = player._authUser
        ? Math.max(Number(player.xp) || 0, Number(data.xp))
        : Math.max(0, Number(data.xp));
    }
    if (Number.isFinite(Number(data.sc ?? data.score))) {
      player.score = Math.max(0, Number(data.sc ?? data.score));
  ...
  socket.on('eat_apple', () => {
    ...
    socket.emit('self_state', { hp: player?.hp ?? 250, hpSeq: player?.hpSeq || 0, hpAt: player?.hpAt || Date.now() });
  });
  ...
  socket.on('mob_hit_req', (data = {}) => {
    ...
    if (mob.hp <= 0) {
      ...
      const rewardScore = Math.round((mob.xpReward || 35) * 0.75 + rewardGold * 3);
      attacker.gold = (attacker.gold || 0) + rewardGold;
      attacker.score = (attacker.score || 0) + rewardScore;
      persistPlayerScore(attacker);
      socket.emit('self_state', { g: attacker.gold, sc: attacker.score });
